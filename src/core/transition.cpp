...
std::vector<Transition> make_transitions(
    ...
    if (observations.size() != n || actions.size() != n ||
        step_result.rewards.size() != n || step_result.terminated.size() != n ||
        step_result.truncated.size() != n || step_result.infos.size() != n ||
        step_result.final_observations.size() != n) {
        throw std::invalid_argument(
            "make_transitions(): every input and VectorStepResult column must "
            "match the sub-env count (" + std::to_string(n) + ")");
    }

    std::vector<Transition> transitions;
    transitions.reserve(n);
    for (size_t i = 0; i < n; ++i) {
        const bool episode_ended =
            step_result.terminated[i] != 0 || step_result.truncated[i] != 0;
        if (step_result.final_observations[i].has_value() != episode_ended) {
            throw std::invalid_argument(
                "make_transitions(): final_observations must be present exactly "
                "for terminated or truncated sub-environments");
        }
        Transition transition;
